...
- [~] **doc 25 P2 — Năng lượng + Campaign — CODE XONG (chờ verify tay DB/3D)** (2026-08-18). Kế hoạch: [28-phase2-energy-campaign-impl.md](28-phase2-energy-campaign-impl.md). E1 catalog Campaign (5 cấp, shared thuần) · E2 power-up→modifier (speed/head_start) · E2b mạng phụ + trạng thái `lost` (hết mạng=thua) · E3 năng lượng server-authoritative (migration+RPC read/spend/grant + `GET /v1/energy`, kinh tế 50/180s) · E4 tiến độ Campaign (migration `campaign_plays` nonce + `player_level_progress` + RPC start/complete + `CampaignController`, unlock/thưởng server-side) · E5 client `/campaign` (lưới cấp, thanh năng lượng, power-up) · E6 HUD objective + màn thua + nộp kết quả. Test 203→**233** (shared 121/server 54/client 58); typecheck+build xanh; smoke DOM `/campaign` OK. **CHỜ:** áp 2 migration Supabase + smoke RPC; chạy backend+client thử vòng Campaign đủ; kiểm 3D obstacle/head_start. Hoãn P3: schema Level trên Supabase + admin API + trình vẽ hex trực quan.
