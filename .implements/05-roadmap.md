...
- [~] **doc 25 P2 — Năng lượng + Campaign — CODE XONG (chờ verify tay DB/3D)** (2026-08-18). Kế hoạch: [28-phase2-energy-campaign-impl.md](28-phase2-energy-campaign-impl.md). E1 catalog Campaign (5 cấp, shared thuần) · E2 power-up→modifier (speed/head_start) · E2b mạng phụ + trạng thái `lost` (hết mạng=thua) · E3 năng lượng server-authoritative (migration+RPC read/spend/grant + `GET /v1/energy`, kinh tế 50/180s) · E4 tiến độ Campaign (migration `campaign_plays` nonce + `player_level_progress` + RPC start/complete + `CampaignController`, unlock/thưởng server-side) · E5 client `/campaign` (lưới cấp, thanh năng lượng, power-up) · E6 HUD objective + màn thua + nộp kết quả. **+ đăng nhập DEV local** (`POST /v1/auth/dev`, chặn production; nút "Đăng nhập dev" chỉ ở localhost). **+ A (đóng P2):** sao thật `campaignStars(deaths)` + điểm (%×10); **mua năng lượng bằng coin** (migration 003 `energy_purchase`: giá ở `energy_rules`, `purchase_energy_with_coin` idempotent, `POST /v1/energy/purchase`, nút Mua; dev-login tặng 1000 coin). Test 203→**235** (shared 122/server 55/client 58); typecheck+build xanh. **Đã verify:** đăng nhập dev + chơi Campaign OK (bạn xác nhận). Nhánh `phase1/modes` đã **gộp `main`** (fast-forward).
- [~] **doc 29 P3 — Schema Level Supabase + Admin API + trình vẽ hex — CODE XONG L1-L5 (chờ verify tay)** (2026-08-18). Kế hoạch: [29-phase3-level-authoring-plan.md](29-phase3-level-authoring-plan.md). L1 migration `campaign_levels` (jsonb config/rewards, published/version) + seed 5 cấp (id c1..c5) khớp hằng → progress cũ bất biến · L2 `GET /v1/campaign/levels` + client fetch (fallback hằng shared) · L3 start/complete đọc unlock+thưởng TỪ DB (`isUnlockedIn`) · L4 Admin API CRUD (`internal/v1/admin/levels`, RPC `upsert/publish_campaign_level`, `validateLevelDraft` shared) · L5 trình vẽ hex `/admin/levels` (form + lưới SVG tô obstacle + validate + publish + Xem thử). Test 235→**237** (shared 125/server 56/client 58); typecheck+build xanh; route `/admin/levels` sinh OK. **CHỜ:** áp **migration 003+004+005** lên Supabase; đặt `ADMIN_API_KEY_SHA256` (sha256 của admin key) để dùng trình vẽ; restart dev server (nạp route mới); smoke tạo/publish 1 cấp qua trình vẽ → hiện ở `/campaign`. Hoãn: L6 (tách app admin riêng), custom-cells/map lõm, StarCriteria dữ liệu.
