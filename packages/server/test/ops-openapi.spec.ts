...
describe("isDryRun — có mặt là bật, trừ khi phủ định rõ ràng", () => {
  it("bật với 'true'", () => {
    expect(isDryRun({ query: { dry_run: "true" } } as never)).toBe(true);
  });

  it("bật với 1/yes/on/TRUE/rỗng — vì hiểu nhầm thành THẬT là hướng không hoàn tác được", () => {
    // Bản đầu trả false cho những giá trị này, với lập luận "gõ nhầm sẽ bị chặn vì thiếu
    // Idempotency-Key". Lá chắn đó thủng đúng với người gọi cẩn thận nhất: agent làm theo tài liệu
    // LUÔN gửi Idempotency-Key cho lời gọi ghi, nên `?dry_run=1` của họ qua guard và chạy thật.
    for (const v of ["1", "yes", "on", "TRUE", " true ", "", undefined]) {
      expect(isDryRun({ query: { dry_run: v } } as never), String(v)).toBe(true);
    }
  });

  it("tắt khi VẮNG MẶT, và khi phủ định rõ ràng", () => {
    // `?dry_run=false` là cách nói "tôi biết tham số này tồn tại và tôi muốn làm thật".
