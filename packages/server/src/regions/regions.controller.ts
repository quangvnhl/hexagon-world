...
interface TicketBody { region?: string; guestId?: string; displayName?: string; appearance?: Partial<PlayerAppearance>; }
...
export class RegionsController {
  ...
  async authenticated(@Req() req: Request, @Body() body: TicketBody) {
    ...
    const savedAppearance = { colorIndex: Number(String(data?.selected_color ?? "color:0").split(":")[1] ?? 0), shape: String(data?.selected_shape ?? "shape:cube").replace("shape:", "") as never, trailPattern: String(data?.selected_trail_pattern ?? "trail:solid").replace("trail:", "") as never };
    const appearance = sanitizePlayerAppearance(body.appearance ?? savedAppearance);
