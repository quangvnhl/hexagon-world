...
function runtimeConfig() {
    if (cached)
        return cached;
    const role = text("SERVER_ROLE", "all");
    if (!["all", "control", "game"].includes(role))
        throw new Error("SERVER_ROLE không hợp lệ");
    ...
    if (regionsRaw) {
        try {
            ...
            throw new Error("GAME_REGIONS_JSON không phải JSON hợp lệ");
        }
    }
    if (!regions.length) {
        regions = [{
    ...
    if (role !== "all" && !cached.gameResultSecret)
        throw new Error("Deployment tách riêng cần GAME_RESULT_SECRET");
    return cached;
}
function sha256(value) {
    return (0, node_crypto_1.createHash)("sha256").update(value).digest("hex");
}
function resetRuntimeConfigForTests() { cached = null; }
