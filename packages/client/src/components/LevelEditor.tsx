...
// Trình vẽ hex cho admin (doc 29 §L5): tô ô chướng ngại + form objective/thưởng/unlock → publish.
// Gate bằng admin key (nhập tay, gửi header x-admin-key). Tái dùng toán hex + validateLevelDraft.
...
export default function LevelEditor() {
  ...
  // Kích thước viewBox của lưới sửa.
  ...
  if (previewing) {
    return <GameScene config={buildConfig(form, obstacles)} endMode="campaign" onExit={() => setPreviewing(false)} />;
  }
        ...
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1.1fr)", gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              ...
              <button onClick={() => setPreviewing(true)} style={btn("rgba(255,210,63,0.15)")}>▶ Xem thử</button>
            </div>
          </div>
            ...
            <svg viewBox={`${bounds.minX} ${bounds.minY} ${bounds.w} ${bounds.h}`} style={{ width: "100%", background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.08)" }}>
              {EDIT_CELLS.map((c) => {
                ...
                return <polygon key={`${c.q},${c.r}`} points={hexPoints(c.cx, c.cy, EDIT_SIZE - 1)}
                  onClick={() => toggleCell(c.q, c.r)}
                  style={{ cursor: "pointer", fill: on ? "#ff6a5a" : center ? "rgba(120,220,150,0.25)" : "rgba(120,140,180,0.12)", stroke: "rgba(255,255,255,0.15)", strokeWidth: 0.5 }} />;
              })}
            </svg>
